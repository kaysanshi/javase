...
public class ChatClientFrame extends JFrame {
    private JTextField tf_newUser;//登录时用
    private JList user_list;
    private JTextArea ta_info;
    private JTextField tf_send;//发送时用
    ...
    public void createClientSocket() {
        try {
            Socket socket = new Socket("202.196.37.239",1978);// 创建套接字对象
...
class ClientThread extends Thread {
     ...
     public void run() {
         try {
             ...
             // 获得列表框的模型
              DefaultComboBoxModel model = (DefaultComboBoxModel) user_list.getModel();
            while (true) {
