...
public class ProductDao {
	...
	public void add(Product product){
		...
		Object params[]={
				...
				product.getQuality(),product.getHit(),product.getTime(),
...
//		String sql = "SELECT p.productid,p.content,p.hit,p.markprice,p.name,p.photo," +
//				"p.price,p.productid,p.quality,p.time,c.name AS cname" +
...
//		return JDBCUtils.select(sql, startIndex,size);
	...
	//获取记录列表,带查询条件的
		public List<Map<String,Object>> findAll(int startIndex,int size,String skey,String svalue){
			//SELECT p.productid,... AS cname FROM product AS p,category AS c WHERE p.categoryid=c.categoryid AND p.NAME LIKE '%1%' LIMIT 0,3
			String sql = "SELECT p.productid,p.content,p.hit,p.markprice,p.name,p.photo," +
					"p.price,p.productid,p.quality,p.time,c.name AS cname" +
					" FROM product AS p,category AS c WHERE p.categoryid=c.categoryid";
			StringBuilder str = new StringBuilder("");
			if(skey!=null&&(skey.trim().length()>0)&&svalue!=null&&(svalue.trim().length()>0)){
				//有查询条件, AND p.NAME LIKE '%1%' 
	...
	public void update(Product product){
		String sql = "update product set name=?,price=?,markprice=?," +
				"quality=?,hit=?,time=?,photo=?,content=?,categoryid=? where productid=?";
		Object params[]={
				product.getName(),product.getPrice(),product.getMarkprice(),
				product.getQuality(),product.getHit(),product.getTime(),
				product.getPhoto(),product.getContent(),product.getCategoryid(),
	...
	//获取记录列表,带查询条件和排序
			public List<Map<String,Object>> findAll(int startIndex,int size,String skey,String svalue,String sortkey,String sort){
				...
				if(sortkey!=null&&(sortkey.trim().length()>0)&&sort!=null&&(sort.trim().length()>0)){
					//有排序条件,SELECT * FROM product ORDER BY price ASC LIMIT 0,12
