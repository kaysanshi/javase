...
<body>
	...
	<div class="rightinfo">
		...
		<table class="tablelist" >
			<thead>
				<tr>
					<th><input id="all" type="checkbox" value="" />
					</th>
			...
			<c:forEach var="item" items="${page.list}">
				<tr>
					...
					<td><a href="<c:url value='/adminProductServlet?action=find&id=${item.productid}'/> " class="tablelink">查看</a> <a
						href="<c:url value='/adminProductServlet?action=updatebefore&id=${item.productid}'/> " class="tablelink"> 修改</a>
			...
			<ul class="paginList">
				<c:choose>
					...
					<c:otherwise>
						<li class="paginItem">
						<a href="
						<c:url value='/adminProductServlet?action=findAll&current=${page.currentPage-1}&skey=${skey}&svalue=${svalue}'/> "><span
						class="pagepre"></span>
						</a>
						</li>
					</c:otherwise>
				...
				<c:forEach var="i" begin="${page.start}" end="${page.end}">
					<c:choose>
						...
						<c:otherwise>
							...
							<c:url value='/adminProductServlet?action=findAll&current=${i}&skey=${skey}&svalue=${svalue}'/>">${i}</a>
							</li>
						</c:otherwise>
					</c:choose>
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="paginItem">
						<a href="
						<c:url value='/adminPrductServlet?action=findAll&current=${page.currentPage+1 }&skey=${skey}&svalue=${svalue}'/> "><span
						class="pagenxt"></span>
						</a>
						</li>
					</c:otherwise>
