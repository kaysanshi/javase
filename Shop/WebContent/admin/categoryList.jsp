			...
			<c:forEach var="item" items="${page.list}">
				<tr>
					...
					<td><a href="<c:url value='/adminCategoryServlet?action=updatebefore&id=${item.categoryid}'/> " class="tablelink"
						...
						<c:url value="/adminCategoryServlet?action=delete&id=${item.categoryid}"/>';" class="click"> 删除</a>
					</td>
				</tr>
				</c:forEach>
			</tbody>
			...
			<ul class="paginList">
				<c:choose>
					...
					<c:otherwise>
						<li class="paginItem">
						<a href="
						<c:url value='/adminCategoryServlet?action=findAll&current=${page.currentPage-1 }'/> "><span
						class="pagepre"></span>
						</a>
						</li>
					</c:otherwise>
				...
				<c:forEach var="i" begin="${page.start}" end="${page.end}">
					<c:choose>
						...
						<c:otherwise>
							...
							<c:url value='/adminCategoryServlet?action=findAll&current=${i}'/>">${i}</a>
							</li>
						</c:otherwise>
					</c:choose>
